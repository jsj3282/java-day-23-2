package ex06.commonService.membership;

import java.io.IOException;
import java.util.ArrayList;

import ex06.Controller6;
import ex06.Member;
import ex06.MemberInformationController;
...
public class MemberServiceImpl implements MembershipService {
	...
	private void viewMember() {
		boolean bool = Controller6.db.insert(mem);
		if(bool) {
		...
		Stage membershipInformation = new Stage();
		Parent root = null;
		FXMLLoader loader = new FXMLLoader(getClass().getResource("/ex06/MemberInformation.fxml"));
		try {
			root = loader.load();
		} catch (IOException e) {
			e.printStackTrace();
		}
		membershipInformation.setScene(new Scene(root));
		MemberInformationController ctrler = loader.getController();
		ctrler.setMember(root, mem);
		membershipInformation.show();
	}
